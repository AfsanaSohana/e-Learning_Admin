...
function ClassesAdd() {
      const [inputs, setInputs] = useState({id:'',class_name:''});
    const navigate=useNavigate();
    const {id} = useParams();
    
    function getDatas(){
        axios.get(`${process.env.REACT_APP_API_URL}/classes/${id}`).then(function(response) {
    ...
    const handleSubmit = async(e) => {
        ...
        try{
            let apiurl='';
            if(inputs.id!=''){
                apiurl=`/classes/edit/${inputs.id}`;
            }else{
                apiurl=`/classes/create`;
            }
